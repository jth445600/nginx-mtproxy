...
read -rp "你需要TAG标签吗(Y/N): " chrony_install
    ...
    [yY][eE][sS] | [yY])
        ...
        docker run --name nginx-mtproxy -d -e tag="$tag" -e secret="$secret" -e domain="$domain" -e ip_white_list="IP"  -p 80:80 -p $port:$port ellermister/nginx-mtproxy:latest
        ;;
    *)
echo -e "正在安装依赖: Docker... "
...
echo -e "正在安装nginx-mtproxy... "
docker run --name nginx-mtproxy -d -e secret="$secret" -e domain="$domain" -p 8090:80 -p $port:443 ellermister/nginx-mtproxy:latest
        ;;
    esac
